...
Buffer::Buffer(int cap) : _cap(cap) {
    _buffer = new char[_cap]; ::memset(_buffer, 0, _cap);
}

Buffer::Buffer &operator=(const Buffer &) = delete;
...
int Buffer::readNioToBufferTillEnd(int fd, const char *end, int step = _default_step) {
    assert(step > 0);
...
void Buffer::writeNioFromBufferTillEnd(int fd, int step = _default_step) {
    while (_start != _offset) {
        if (step > _offset - _start) { step = _offset - _start; }
        int curr = writeNioFromBuffer(fd, step);
